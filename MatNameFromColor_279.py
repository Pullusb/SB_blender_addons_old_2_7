...
def convert_color_to_name(color):
    '''get color as a lsit or tuple and return closest english name'''
...
#recurive find color in node_tree
...
def dl_popup_panel(self, context):
    layout = self.layout
    layout.label("This functionality use module webcolors")
    layout.label("(manual installation notes are diplayed in console)")
    layout.label("Install automatically ? (restart blender)")
...
class AutoNameMaterial(bpy.types.Operator):
...
class ViewportColorFromNode(bpy.types.Operator):
...
def register():
    ...
    bpy.types.CyclesMaterial_PT_settings.append(MaterialPlus)
 
def unregister():
    bpy.types.CyclesMaterial_PT_settings.remove(MaterialPlus)
